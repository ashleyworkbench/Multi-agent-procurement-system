...
function KafkaLogRow({ log }: { log: any }) {
  ...
  const cfg = eventConfig[log?.event_type] ?? { color: "text-slate-500 bg-slate-50", icon: Zap, label: log?.event_type || "Event" };
  const Icon = cfg.icon;
    ...
    <div className="border border-slate-100 rounded-xl overflow-hidden hover:border-slate-200 transition-colors">
      ...
      >
        ...
        <span className={cn("badge text-[11px]", agentColor[log?.source_agent] ?? "badge-slate")}>
          {log?.source_agent?.replace("_", " ") || "Agent"}
        </span>
...
function AgentStatusBadge({ name, fetchFn }: { name: string; fetchFn: () => Promise<any> }) {
  ...
  const running = data?.status === "running" || data?.status === "active" || data?.status === "ready" || (!isError && data?.status && data.status !== "offline" && data.status !== "degraded");
  return (
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {filtered.map((log: any, idx: number) => (
            <KafkaLogRow key={log?.event_id || log?.id || idx} log={log} />
          ))}
        </div>
      )}
    </div>
